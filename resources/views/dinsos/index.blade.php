...
@endif
<a href="{{ route('dinsos.create') }}" class="btn btn-primary mb-3 col-12">Tambah</a>
<table class="table table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="align-middle">
                <a href="{{ route('dinsos.edit', $item->kode)  }}">
                    <button type="button" class="btn btn-warning text-white col">Ubah</button>
                </a>
            </td>
            <td class="align-middle">
                <form method="post" action="{{ route('dinsos.destroy', $item->kode) }}">
                    @method('delete')
                    @csrf
...
<form method="post" action="{{ route('dinsos.store') }}" enctype="multipart/form-data">
    @csrf
    @include('dinsos/_form')
</form>
@elseif (Str::of(Route::currentRouteName())->after('.') == 'edit')
<form method="post" action="{{ route('dinsos.update', $dinsos->kode) }}" enctype="multipart/form-data">
    @csrf
    @method('put')
    @include('dinsos/_form')
</form>
